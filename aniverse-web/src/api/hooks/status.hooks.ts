    ...
    return useServerMutation<string, { profileType: "heap" | "allocs" }>({
        ...
        onMutate: async (variables) => {
            ...
            try {
                ...
                if (password) {
                    headers["X-Aniverse-Token"] = password
                }
                ...
                const filename = `aniverse-${profileType}-profile-${timestamp}.pprof`
    ...
    return useServerMutation<string>({
        ...
        onMutate: async () => {
            ...
            try {
                ...
                const timestamp = new Date().toISOString().replace(/[:.]/g, "-").split("T")[0] + "_" +
                    new Date().toISOString().replace(/[:.]/g, "-").split("T")[1].split(".")[0]
                const filename = `aniverse-goroutine-profile-${timestamp}.pprof`
    ...
    return useServerMutation<string, { duration?: number }>({
        ...
        onMutate: async (variables) => {
            ...
            try {
                ...
                const timestamp = new Date().toISOString().replace(/[:.]/g, "-").split("T")[0] + "_" +
                    new Date().toISOString().replace(/[:.]/g, "-").split("T")[1].split(".")[0]
                const filename = `aniverse-cpu-profile-${timestamp}.pprof`
