...
const generateBridgeScript = (token: string, parentOrigin: string, widgetId: string) => `
    ...
    window.webview = {
        ...
        // Receive messages from Aniverse
        on: (event, callback) => {
            const handler = (e) => {
        ...
        // Notify Aniverse when the webview body changed size
        _onResizeObserved: () => {
    ...
    // Notify Aniverse to resize iframe when content changes
