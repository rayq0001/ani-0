...
func InitRoutes(app *core.App, e *echo.Echo) {
	...
	//
	v1AI := v1.Group("/ai")
	v1AI.GET("/settings", h.HandleGetAISettings)
	v1AI.POST("/settings", h.HandleUpdateAISettings)
	v1AI.POST("/ocr", h.HandleOCRPage)
	v1AI.POST("/translate", h.HandleTranslateText)
	v1AI.POST("/chat", h.HandleAIChat)
	v1AI.POST("/recap", h.HandleGenerateRecap)
	v1AI.POST("/search", h.HandleAIVibeSearch)
	v1AI.POST("/lore", h.HandleGenerateLoreTree)
	v1AI.POST("/upscale", h.HandleUpscaleImage)
	...
	// Offline Downloads
	v1Anyverse.POST("/offline/queue", h.HandleQueueDownload)
	v1Anyverse.POST("/offline/queue-next", h.HandleQueueNextChapters)
	v1Anyverse.GET("/offline/queue", h.HandleGetDownloadQueue)
	v1Anyverse.GET("/offline/active", h.HandleGetActiveDownloads)
	v1Anyverse.GET("/offline/completed", h.HandleGetCompletedDownloads)
	v1Anyverse.POST("/offline/pause", h.HandlePauseDownload)
	v1Anyverse.POST("/offline/resume", h.HandleResumeDownload)
	v1Anyverse.POST("/offline/cancel", h.HandleCancelDownload)
	v1Anyverse.POST("/offline/clear-completed", h.HandleClearCompletedDownloads)
	v1Anyverse.GET("/offline/stats", h.HandleGetDownloadStats)
	// Cultural Localization
	v1Anyverse.POST("/culture/localize", h.HandleCulturalLocalization)
	v1Anyverse.POST("/culture/voice", h.HandleVoiceCustomization)
	// AI Director
	v1Anyverse.POST("/director/generate", h.HandleAIDirector)
	// Emotional OST
	v1Anyverse.POST("/ost/generate", h.HandleGenerateOST)
	// Subscription & Wallet
	v1Anyverse.GET("/subscription", h.HandleGetSubscription)
	v1Anyverse.POST("/subscription/upgrade", h.HandleUpgradeSubscription)
	v1Anyverse.GET("/wallet/balance", h.HandleGetWalletBalance)
	v1Anyverse.POST("/wallet/purchase", h.HandlePurchaseCoins)
	// Health Check
	v1Anyverse.GET("/health", h.HandleGetHealthStatus)
	// Space / Unlimited Space
	v1Anyverse.POST("/cosmic-search", h.HandleCosmicSearch)
	v1Anyverse.GET("/galaxy-clusters", h.HandleGetGalaxyClusters)
	v1Anyverse.GET("/similar-galaxies/:id", h.HandleGetSimilarGalaxies)
