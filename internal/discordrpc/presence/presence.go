...
var (
	defaultActivity = discordrpc_client.Activity{
		...
		Assets: &discordrpc_client.Assets{
			LargeImage: "",
			LargeText:  "",
			SmallImage: "https://aniverse.app/images/circular-logo.png",
			SmallText:  "aniverse v" + constants.Version,
			SmallURL:   "https://aniverse.app",
		},
...
func isaniverseButtonPresent(activity *discordrpc_client.Activity) bool {
	if activity == nil || activity.Buttons == nil {
		return false
	}
	for _, button := range activity.Buttons {
		if button.Label == "aniverse" && button.Url == "https://aniverse.app" {
			return true
		}
	}
	return false
...
func (p *Presence) SetAnimeActivity(a *AnimeActivity) {
	...
	if !(p.settings.RichPresenceHideaniverseRepositoryButton || len(activity.Buttons) > 1) {
		activity.Buttons = append(activity.Buttons, &discordrpc_client.Button{
			Label: "aniverse",
			Url:   "https://aniverse.app",
		})
	...
	// Only allow changing small image and text if aniverse button is present
	if isaniverseButtonPresent(&activity) {
